...
cGameObject::constructorCommon()
{
    ...
    tSigned32 i = _initTemplate->firstChildWithName("toggled");
    if(i >= 0)
    {
                        ...
                        const cSimpleDOM& templates, const std::string& contentName, float orientation)
{
    ...
    _agent->setUserData(reinterpret_cast<tSigned64>(this));
}

cGameObject::cGameObject(tSigned32 index, const cSimpleDOM& templates, const std::string& contentName, iAgent* agent)
{
    _index = index;
    setContentName(contentName);
    tSigned32 i = templates.firstChildWithName(_type);
...
iPath*
cGameObject::findPathAway(const cPosition& awayFrom, tSigned32 distanceAway) const
{
...
cGameObject::moveUnderDirectControl(float localX, float localY, float speedMultiplier)
{
    ...
    tSigned32 dxL = static_cast<tSigned32>(dx);
    tSigned32 dyL = static_cast<tSigned32>(dy);
...
cGameObject::moveInDirection(float dX, float dY)
{
    ...
    cPosition targetP;
    targetP.x = static_cast<tSigned32>(currentP.x + offsetX);
    targetP.y = static_cast<tSigned32>(currentP.y + offsetY);
    ...
    if(!collided)
    {
        ...
        tSigned32 offsetXL = targetP.x - currentP.x;
        tSigned32 offsetYL = targetP.y - currentP.y;
...
cGameObject::setDefaultBehaviour()
{
    ...
    tSigned32 i = _initTemplate->firstChildWithName("defaultBehaviour");
    if(i >= 0)
    {
