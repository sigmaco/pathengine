        ...
        )
{
    ...
    tSigned32 i;
    for(i = 1; i != static_cast<tSigned32>(placedInstances.size()); ++i)
    {
        ...
        )
{
    ...
    tSigned32 i;
    for(i = 0; i != static_cast<tSigned32>(placedInstances.size()); ++i)
    {
        cHorizontalRange instanceRange;
        placedInstances[i]->getHorizontalRange(instanceRange);
...
//    snapshotOSPtr = &snapshotFOS;
    ...
    iMesh* tileMesh = federation->buildTileMeshFromContent(
            tileIndex,
            instancesBuffer, static_cast<tSigned32>(overlappingInstances.size()),
