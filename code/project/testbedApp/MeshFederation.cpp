...
static iMesh*
LoadTileMesh(iTestBed* testBed, iPathEngine* pathEngine, tSigned32 tileIndex)
{
    ...
    if(size)
    {
        result = pathEngine->loadMeshFromBuffer("tok", buffer, size, 0);
    }
...
GetUnobstructedPositionInFederation(
        ...
        iShape* agentShape,
        tSigned32 agentSize,
        tSigned32& tileIndex, cPosition& positionInTile
        )
{
...
// and save this all to disk
    ...
    const tSigned32 tileSize = 8000;
    const tSigned32 overlap = 2000;
    const tSigned32 agentSize = 20;
    ...
    //const tSigned32 tileSize = 40000;
    //const tSigned32 overlap = 20000;
    //const tSigned32 agentSize = 160;
    ...
    //const tSigned32 overlap = 20000;
    //const tSigned32 agentSize = 160;
    //BuildFederation_TileByTile3DCP(pathEngine, testBed, tileSize, overlap);
...
// and run the demo against these loaded objects
  ...
  // display the meshes in the testbed while the preprocess is being generated
    ...
    std::vector<iMesh*> tileMeshes(federation->size(), (iMesh*)0);
    std::vector<iCollisionContext*> contexts(federation->size(), (iCollisionContext*)0);
    tSigned32 i;
